...
impl MD5Machine<'_> {
    ...
    fn rounds(&mut self) {
        ...
        Self::ff(&mut d, a, b, c, x[1], 12, 0xe8c7b756); /* 2 */
        Self::ff(&mut c, d, a, b, x[2], 17, 0x242070db); /* 3 */
        Self::ff(&mut b, c, d, a, x[3], 22, 0xc1bdceee); /* 4 */
        Self::ff(&mut a, b, c, d, x[4], 7, 0xf57c0faf); /* 5 */
        Self::ff(&mut d, a, b, c, x[5], 12, 0x4787c62a); /* 6 */
        Self::ff(&mut c, d, a, b, x[6], 17, 0xa8304613); /* 7 */
        Self::ff(&mut b, c, d, a, x[7], 22, 0xfd469501); /* 8 */
        Self::ff(&mut a, b, c, d, x[8], 7, 0x698098d8); /* 9 */
        Self::ff(&mut d, a, b, c, x[9], 12, 0x8b44f7af); /* 10 */
        Self::ff(&mut c, d, a, b, x[10], 17, 0xffff5bb1); /* 11 */
        Self::ff(&mut b, c, d, a, x[11], 22, 0x895cd7be); /* 12 */
        Self::ff(&mut a, b, c, d, x[12], 7, 0x6b901122); /* 13 */
        Self::ff(&mut d, a, b, c, x[13], 12, 0xfd987193); /* 14 */
        Self::ff(&mut c, d, a, b, x[14], 17, 0xa679438e); /* 15 */
        Self::ff(&mut b, c, d, a, x[15], 22, 0x49b40821); /* 16 */
        Self::gg(&mut a, b, c, d, x[1], 5, 0xf61e2562); /* 17 */
        Self::gg(&mut d, a, b, c, x[6], 9, 0xc040b340); /* 18 */
        Self::gg(&mut c, d, a, b, x[11], 14, 0x265e5a51); /* 19 */
        Self::gg(&mut b, c, d, a, x[0], 20, 0xe9b6c7aa); /* 20 */
        Self::gg(&mut a, b, c, d, x[5], 5, 0xd62f105d); /* 21 */
        Self::gg(&mut d, a, b, c, x[10], 9, 0x2441453); /* 22 */
        Self::gg(&mut c, d, a, b, x[15], 14, 0xd8a1e681); /* 23 */
        Self::gg(&mut b, c, d, a, x[4], 20, 0xe7d3fbc8); /* 24 */
        Self::gg(&mut a, b, c, d, x[9], 5, 0x21e1cde6); /* 25 */
        Self::gg(&mut d, a, b, c, x[14], 9, 0xc33707d6); /* 26 */
        Self::gg(&mut c, d, a, b, x[3], 14, 0xf4d50d87); /* 27 */
        Self::gg(&mut b, c, d, a, x[8], 20, 0x455a14ed); /* 28 */
        Self::gg(&mut a, b, c, d, x[13], 5, 0xa9e3e905); /* 29 */
        Self::gg(&mut d, a, b, c, x[2], 9, 0xfcefa3f8); /* 30 */
        Self::gg(&mut c, d, a, b, x[7], 14, 0x676f02d9); /* 31 */
        Self::gg(&mut b, c, d, a, x[12], 20, 0x8d2a4c8a); /* 32 */
        Self::hh(&mut a, b, c, d, x[5], 4, 0xfffa3942); /* 33 */
        Self::hh(&mut d, a, b, c, x[8], 11, 0x8771f681); /* 34 */
        Self::hh(&mut c, d, a, b, x[11], 16, 0x6d9d6122); /* 35 */
        Self::hh(&mut b, c, d, a, x[14], 23, 0xfde5380c); /* 36 */
        Self::hh(&mut a, b, c, d, x[1], 4, 0xa4beea44); /* 37 */
        Self::hh(&mut d, a, b, c, x[4], 11, 0x4bdecfa9); /* 38 */
        Self::hh(&mut c, d, a, b, x[7], 16, 0xf6bb4b60); /* 39 */
        Self::hh(&mut b, c, d, a, x[10], 23, 0xbebfbc70); /* 40 */
        Self::hh(&mut a, b, c, d, x[13], 4, 0x289b7ec6); /* 41 */
        Self::hh(&mut d, a, b, c, x[0], 11, 0xeaa127fa); /* 42 */
        Self::hh(&mut c, d, a, b, x[3], 16, 0xd4ef3085); /* 43 */
        Self::hh(&mut b, c, d, a, x[6], 23, 0x4881d05); /* 44 */
        Self::hh(&mut a, b, c, d, x[9], 4, 0xd9d4d039); /* 45 */
        Self::hh(&mut d, a, b, c, x[12], 11, 0xe6db99e5); /* 46 */
        Self::hh(&mut c, d, a, b, x[15], 16, 0x1fa27cf8); /* 47 */
        Self::hh(&mut b, c, d, a, x[2], 23, 0xc4ac5665); /* 48 */
        Self::ii(&mut a, b, c, d, x[0], 6, 0xf4292244); /* 49 */
        Self::ii(&mut d, a, b, c, x[7], 10, 0x432aff97); /* 50 */
        Self::ii(&mut c, d, a, b, x[14], 15, 0xab9423a7); /* 51 */
        Self::ii(&mut b, c, d, a, x[5], 21, 0xfc93a039); /* 52 */
        Self::ii(&mut a, b, c, d, x[12], 6, 0x655b59c3); /* 53 */
        Self::ii(&mut d, a, b, c, x[3], 10, 0x8f0ccc92); /* 54 */
        Self::ii(&mut c, d, a, b, x[10], 15, 0xffeff47d); /* 55 */
        Self::ii(&mut b, c, d, a, x[1], 21, 0x85845dd1); /* 56 */
        Self::ii(&mut a, b, c, d, x[8], 6, 0x6fa87e4f); /* 57 */
        Self::ii(&mut d, a, b, c, x[15], 10, 0xfe2ce6e0); /* 58 */
        Self::ii(&mut c, d, a, b, x[6], 15, 0xa3014314); /* 59 */
        Self::ii(&mut b, c, d, a, x[13], 21, 0x4e0811a1); /* 60 */
        Self::ii(&mut a, b, c, d, x[4], 6, 0xf7537e82); /* 61 */
        Self::ii(&mut d, a, b, c, x[11], 10, 0xbd3af235); /* 62 */
        Self::ii(&mut c, d, a, b, x[2], 15, 0x2ad7d2bb); /* 63 */
        Self::ii(&mut b, c, d, a, x[9], 21, 0xeb86d391); /* 64 */
        // End generated
    ...
    fn op(
        ...
        //println!("foo before {:x}", *a);
        let unrotated = (Wrapping(*a) + Wrapping(f(b, c, d)) + Wrapping(x_k) + Wrapping(t_i)).0;
        *a = (Wrapping(b) + Wrapping((unrotated << s) | (unrotated >> (32 - s)))).0;
        //println!("foo after  {:x}", *a);
    }

    fn ff(a: &mut u32, b: u32, c: u32, d: u32, x_k: u32, s: u8, t_i: u32) {
        Self::op(&Self::aux_f, a, b, c, d, x_k, s, t_i)
    }

    fn gg(a: &mut u32, b: u32, c: u32, d: u32, x_k: u32, s: u8, t_i: u32) {
        Self::op(&Self::aux_g, a, b, c, d, x_k, s, t_i)
    }

    fn hh(a: &mut u32, b: u32, c: u32, d: u32, x_k: u32, s: u8, t_i: u32) {
        Self::op(&Self::aux_h, a, b, c, d, x_k, s, t_i)
    }

    fn ii(a: &mut u32, b: u32, c: u32, d: u32, x_k: u32, s: u8, t_i: u32) {
        Self::op(&Self::aux_i, a, b, c, d, x_k, s, t_i)
    }
}
